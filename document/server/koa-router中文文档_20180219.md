...
* [koa-router](#module_koa-router)
    * [Router](#exp_module_koa-router--Router) ⏏
        ...
        * _instance_
            ...
            * [.routes](#module_koa-router--Router+routes) ⇒ <code>function</code>  作用：启动路由
            ...
            * [.allowedMethods([options])](#module_koa-router--Router+allowedMethods) ⇒ <code>function</code>  作用：当请求出错时的处理逻辑
...
| [opts.prefix] | <code>String</code> | 路径前缀    |
...
创建`router.verb()` 方法, *verb* 是一个HTTP方法，类似于`router.get()` 或 `router.post()`.

路由定义格式为：`router.VERB(PATH, HANDLER)`，其中`router`是一个Router实例，`VERB`是一个HTTP请求方法，`PATH`是服务器上的一个路径，`HANDLER`是用于处理请求的函数。

当匹配到URL的时候，会调用`router.verb()`里定义的回调函数

另外, `router.all()` 能够再次匹配所有的路由。
...
当一个匹配到一个路由的时候，它的路径可以在`ctx._matchedRoute` 中获取，如果它被命名，那么这个名字可以在`ctx._matchedRouteName`获取到。
...
// 会对 "/forums/123/posts" and "/forums/123/posts/123"响应
...
router.get('/', ...); // 会响应 "/users"
router.get('/:id', ...); // 会响应 "/users/:id"
```

#### URL参数

Named route 参数可以铜鼓ctx.params`捕获或者添加：
