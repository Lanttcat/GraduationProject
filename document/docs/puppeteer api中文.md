...
- [`Puppeteer`](#class-puppeteer) 使用 [DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/) 协议与浏览器通信.
- [`Browser`](#class-browser) 拥有多个页面的浏览器实例.
- [`Page`](#class-page) 至少拥有一个frame: main frame. 也可以拥有其它通过 [iframe](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe) or [frame](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/frame) 创造的 frame.
...
Puppeteer 寻找某些 [环境变量](https://en.wikipedia.org/wiki/Environment_variable) 来辅助运行. 这些变量可以再环境变量或者 [npm config](https://docs.npmjs.com/cli/config) 设置.

- `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY` - 定义HTTP代理用来下载和运行Chrominum.
- `PUPPETEER_SKIP_CHROMIUM_DOWNLOAD` - 在安装期间不下载Chromium.
- `PUPPETEER_DOWNLOAD_HOST` - 重写URL的host以下载Chromium

### class: Puppeteer

Puppeteer 模块提供了运行Chromium实例的方法.
以下是使用Puppeteer自动运行的典型方法:
```js
...
- returns: <[Array]<[string]>> 配置Chromium运行的参数.

#### puppeteer.executablePath()
- returns: <[string]> Puppeteer运行所需要的Chromium的安装路径. 如果使用了 [`PUPPETEER_SKIP_CHROMIUM_DOWNLOAD`](#environment-variables)Chromium 可能没有下载.
...
BrowserFetcher 能够下载和管理不同版本的Chromium.
...
- returns: <[Promise]<[Array]<[string]>>> 获取本地所有版本列表
...
当Puppeteer连接到Chromium 实例时，一个Browser会被运行, 通过 [`puppeteer.launch`](#puppeteerlaunchoptions) 或者[`puppeteer.connect`](#puppeteerconnectoptions).
...
puppeteer.launch().then(async browser => {
  // 保存一个断点用来重新连接Chromium
  const browserWSEndpoint = browser.wsEndpoint();
  // 断开puppeteer与Chromium的连接
  browser.disconnect();
...
Page类提供了去操作Chromium里的标签页的方法. 一个 [Browser] 实例也许含有多个 [Page] 实例.

例子：创建一个page, 跳转到指定URL, 并且保存了截图:
```js
const puppeteer = require('puppeteer');
...
The Page类会触发某些事件 (详情如下) which can be handled using any of Node's native [`EventEmitter`](https://nodejs.org/api/events.html#events_class_eventemitter) methods, such as `on` or `once`.
...
当JavaScript调用对话框时触发, 如 `alert`, `prompt`, `confirm` or `beforeunload`. Puppeteer can respond to the dialog via [Dialog]'s [accept](#dialogacceptprompttext) or [dismiss](#dialogdismiss) methods.
...
#### page.$(selector)
- `selector` <[string]> 页面选择器
- returns: <[Promise]<?[ElementHandle]>>

这个方法将会在页面中调用 `document.querySelector`. 如果没有匹配到，将会返回`null`.
...
这个方法将会在页面中调用 `document.querySelectorAll` . I如果没有匹配到，将会返回`[]`.
